...
Фейс-риг создаёт цветные curve-контроллеры и рамки по категориям: Brows, Eyes, Nose, Cheeks, Lips и Jaw. Цвета видны и в Material Preview.

### Апскейл текстур

1. Выделите модели, у которых нужно апскейлить текстуры.
2. В блоке `Апскейл текстур` выберите разрешение, модель апскейла, формат и GPU.
3. Проверьте пути `NCNN`, `Models` и `Output`.
4. Нажмите `Апскейлить текстуры выделенного`.

Аддон берёт только `Image Texture` nodes из материалов выделенных mesh-объектов. Все найденные текстуры приводятся к одному размеру, например `512x512 -> 4096x4096` и `1024x1024 -> 4096x4096`. После апскейла текстуры заменяются в материалах выбранных моделей.

Настройка `Color Space` сохраняется: если normal map была `Non-Color`, апскейленная текстура останется `Non-Color`; обычные RGB-текстуры останутся RGB.

Важно: апскейл текстур использует внешние ncnn-модели и может сильно нагружать CPU/GPU/RAM/VRAM. Используйте на свой страх и риск, особенно на больших текстурах или большом количестве материалов: Blender может зависнуть или закрыться, если системе не хватит ресурсов.
